...
[공통 원본 워크플로](../../commands/prefs.md)를 끝까지 읽고 그대로 따른다. 이 파일은 Claude Code,
Codex, Antigravity가 공유하는 단일 원본이다.

Codex 및 Antigravity에서는 `$ARGUMENTS`를 현재 사용자 요청으로 해석하고, `/devkit:<name>`은 대응하는
스킬(`$devkit-<name>` 또는 `devkit-<name>`)로 바꿔 읽는다. 프로젝트 전용 지침 파일은 Codex 및 Antigravity에서는 `AGENTS.md`,
Claude Code에서는 `CLAUDE.md`다. 원본의 저장 위치 판정, 버전 갱신, 커밋·푸시 및 설치본 갱신 절차를
